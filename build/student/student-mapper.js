...
function mapStudents(students, studentGrades, classTypes) {
    ...
    _.each(students, function (student) {
        var grades = _.filter(studentGrades, function (studentGrade) {
            return studentGrade.hb_id === student.hb_id;
        });
        var sortedGrades = _.sortBy(grades, function (grade) {
            return grade.grade_id;
        });
        var studentGrade = sortedGrades[sortedGrades.length - 1].grade_id;
        var newGrades = [];
        _.each(sortedGrades, function (grade) {
            newGrades.push({
                "date": grade.date,
                "grade": grade.grade_id
            });
        });
        var preferredClassType = _.find(classTypes, function (type) {
