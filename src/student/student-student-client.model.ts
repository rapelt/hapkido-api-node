...
export class StudentStudentClientModel {
    name!: {
        firstname: string,
        lastname: string
    };
    ...
    dbToClient(db: Member, classesDB: Class[]): StudentStudentClientModel {
        const student = new StudentStudentClientModel();
        ...
        student.attendance = classesDB.map((aclass) => {
            return {
                classId: aclass.classId,
                isGrading: aclass.isGrading,
                date: moment(aclass.date),
                classType: aclass.classType.classType
            }
        })
    ...
    clientToDB(student: StudentStudentClientModel, classTypes: ClassType[]): Member {
